...
function is_coming_of_age_day(x::TimeType)
    ...
    return Dates.year(x) >= 2000 && is_january(x) && Dates.dayofweekofmonth(x) == 2 && Dates.dayofweek(x) == Dates.Mon
end
...
function is_marine_day(x::TimeType)
    ...
    return Dates.year(x) >= 2003 && is_july(x) && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end
...
function is_respect_for_the_aged_day(x::TimeType)
    ...
    return Dates.year(x) >= 2003 && is_september(x) && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end
...
function is_sports_day(x::TimeType)
    ...
    return Dates.year(x) >= 2000 && is_october(x) && Dates.dayofweekofmonth(x) == 2 && Dates.dayofweek(x) == Dates.Mon
end

...
function is_coming_of_age_day(x::TimeType)
    ...
    return Dates.year(x) >= 2000 && Dates.month(x) == Dates.Jan && Dates.dayofweekofmonth(x) == 2 && Dates.dayofweek(x) == Dates.Mon
end

function is_national_foundation_day(x::TimeType)
    return Dates.month(x) == Dates.Feb && Dates.day(x) == 11
end

function is_emperors_birthday(x::TimeType)
    # Current Emperor's birthday (Emperor Naruhito, born February 23, 1960)
    return Dates.year(x) >= 2020 && Dates.month(x) == Dates.Feb && Dates.day(x) == 23
end

function is_vernal_equinox_day(x::TimeType)
    # Approximately March 20/21 - using March 20 as approximation
    return Dates.month(x) == Dates.Mar && Dates.day(x) == 20
end

function is_golden_week_start(x::TimeType)
    return Dates.month(x) == Dates.Apr && Dates.day(x) == 29
end
...
function is_marine_day(x::TimeType)
    ...
    return Dates.year(x) >= 2003 && Dates.month(x) == Dates.Jul && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end

function is_mountain_day(x::TimeType)
    return Dates.year(x) >= 2016 && Dates.month(x) == Dates.Aug && Dates.day(x) == 11
end

function is_respect_for_the_aged_day(x::TimeType)
    # Third Monday of September
    return Dates.year(x) >= 2003 && Dates.month(x) == Dates.Sep && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end

function is_autumnal_equinox_day(x::TimeType)
    # Approximately September 23 - using as approximation
    return Dates.month(x) == Dates.Sep && Dates.day(x) == 23
end

function is_sports_day(x::TimeType)
    # Second Monday of October
    return Dates.year(x) >= 2000 && Dates.month(x) == Dates.Oct && Dates.dayofweekofmonth(x) == 2 && Dates.dayofweek(x) == Dates.Mon
end

function is_culture_day(x::TimeType)
    return Dates.month(x) == Dates.Nov && Dates.day(x) == 3
end

function is_labour_thanksgiving_day(x::TimeType)
    return Dates.month(x) == Dates.Nov && Dates.day(x) == 23
end

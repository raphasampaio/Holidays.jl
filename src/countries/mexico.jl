...
function is_constitution_day(x::TimeType)
    ...
    return is_february(x) && Dates.dayofweekofmonth(x) == 1 && Dates.dayofweek(x) == Dates.Mon
end

function is_benito_juarez_birthday(x::TimeType)
    # Third Monday in March
    return is_march(x) && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end
...
function is_revolution_day(x::TimeType)
    ...
    return is_november(x) && Dates.dayofweekofmonth(x) == 3 && Dates.dayofweek(x) == Dates.Mon
end

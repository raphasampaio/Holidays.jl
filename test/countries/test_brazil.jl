...
@testset "Brazil" begin
    ...
    @testset "Acre" begin
        ...
        test_holidays(calendar, holidays, [Date(2024, 1, 23), Date(2024, 3, 8), Date(2024, 6, 15), Date(2024, 9, 5), Date(2024, 11, 17)])
    end

    @testset "Alagoas" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Alagoas())
        test_holidays(calendar, holidays, [Date(2024, 6, 24), Date(2024, 6, 29), Date(2024, 9, 16)])
    end

    @testset "Amapa" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Amapa())
        test_holidays(calendar, holidays, [Date(2024, 3, 19), Date(2024, 9, 13)])
    end

    @testset "Amazonas" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Amazonas())
        test_holidays(calendar, holidays, [Date(2024, 9, 5), Date(2024, 12, 8)])
    end

    @testset "Bahia" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Bahia())
        test_holidays(calendar, holidays, [Date(2024, 7, 2)])
    end

    @testset "Brasilia" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Brasilia())
        test_holidays(calendar, holidays, [Date(2024, 4, 21), Date(2024, 11, 30)])
    end

    @testset "Ceara" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Ceara())
        test_holidays(calendar, holidays, [Date(2024, 3, 19), Date(2024, 3, 25), Date(2024, 8, 15)])
    end

    @testset "EspiritoSanto" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.EspiritoSanto())
        test_holidays(calendar, holidays, [Date(2024, 4, 8)])
    end

    @testset "Goias" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Goias())
        test_holidays(calendar, holidays, [Date(2024, 5, 24), Date(2024, 7, 26), Date(2024, 10, 24)])
    end

    @testset "Maranhao" begin
        calendar = Holidays.Brazil(subdivision = Subdivision.Maranhao())
        test_holidays(calendar, holidays, [Date(2024, 7, 28)])
    end
